...
export function ProductsProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // Defer listeners to improve startup speed on native devices
    const timer = setTimeout(() => {
      ...
      // Guardar funciones de limpieza para cuando el componente se desmonte
      (window as any)._unsubs = [unsubProducts, unsubBanners, unsubPortadas];
    }, 800); // 800ms delay for native smoothness

    return () => {
      clearTimeout(timer);
      if ((window as any)._unsubs) {
        (window as any)._unsubs.forEach((u: any) => u());
      }
    };
  }, []);
